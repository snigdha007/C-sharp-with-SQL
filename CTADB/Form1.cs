...
", stationid);
            ...
            foreach (DataRow row in ds.Tables["TABLE"].Rows)
            {
                msg = string.Format("{0}:{1}'", Convert.ToString(row["StopID"]),
                Convert.ToString(row["Name"]));
        ...
        private void top10ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            db = new SqlConnection(connectionInfo);
            db.Open();
            string sql = string.Format(@"select name from stations where stationid =any(select top 10 StationID from Riderships group by StationID order by sum(dailytotal));", this.listBox1.Text);
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = db;
        ...
        private void listBox5_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string msg;
            string select = this.listBox5.SelectedItem.ToString();
            string sql = string.Format(@"SELECT Color FROM Lines ;", select);
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = db;
